...
namespace Pulsebar.Models
{
	public class GraphModel : INotifyPropertyChanged, IDisposable
	{
		public GraphModel(PlotView plot)
		{
			_plot = plot;

			_model = new PlotModel();
			_model.Axes.Add(new LogarithmicAxis { Position = AxisPosition.Left, MajorGridlineStyle = LineStyle.Solid });
			_model.Axes.Add(new DateTimeAxis { Position = AxisPosition.Bottom, StringFormat = "T", Angle = 45 });
			_plot.Model = _model;
		}
		...
		private PlotView _plot { get; set; }

		private PlotModel _model { get; set; }
